...
function draw() {
  ...
  // ── Side-length labels on canvas for selected polygon room ──────────────────
  if (mode === 'select' && selectedRoom && selectedRoom.isPolygon && selectedRoom.points) {
    ...
    // Draw corner letter labels (A, B, C ...)
    pts.forEach((p, i) => {
      ...
      ctx.beginPath();
      ctx.arc(p.x, p.y, (fs * 0.75), 0, Math.PI * 2);
    ...
    for (let i = 0; i < n; i++) {
      ...
      ctx.beginPath();
      ctx.roundRect
        ? ctx.roundRect(mx - tw/2 - pad, my - fs/2 - pad, tw + pad*2, fs + pad*2, 3/zoom)
        : ctx.rect(mx - tw/2 - pad, my - fs/2 - pad, tw + pad*2, fs + pad*2);
      ctx.fill();
      ctx.fillStyle = '#ffffff';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
